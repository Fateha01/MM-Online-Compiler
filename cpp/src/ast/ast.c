...
static char *node_label(const ASTNode *node, int typed, char *buf, size_t bufsz) {
    switch (node->type) {
        ...
        case NODE_DECL:
            ...
            break;
        case NODE_ASSIGN: snprintf(buf, bufsz, "<=>"); break;
        ...
        case NODE_BINOP:
            if (typed) snprintf(buf, bufsz, "<%s> : %s", optype_to_string(node->data.binop.op), datatype_to_string(node->data_type));
            else snprintf(buf, bufsz, "<%s>", optype_to_string(node->data.binop.op));
            break;
        case NODE_UNOP:
            if (typed) snprintf(buf, bufsz, "<%s> : %s", optype_to_string(node->data.unop.op), datatype_to_string(node->data_type));
            else snprintf(buf, bufsz, "<%s>", optype_to_string(node->data.unop.op));
            break;
        case NODE_IDENT:
            if (typed) snprintf(buf, bufsz, "<%s> : %s", node->data.ident.name, datatype_to_string(node->data_type));
            else snprintf(buf, bufsz, "<%s>", node->data.ident.name);
            break;
        case NODE_INT_LITERAL:
            if (typed) snprintf(buf, bufsz, "<%d> : %s", node->data.int_literal.value, datatype_to_string(node->data_type));
            else snprintf(buf, bufsz, "<%d>", node->data.int_literal.value);
            break;
        case NODE_FLOAT_LITERAL:
            if (typed) snprintf(buf, bufsz, "<%g> : %s", node->data.float_literal.value, datatype_to_string(node->data_type));
            else snprintf(buf, bufsz, "<%g>", node->data.float_literal.value);
            break;
        case NODE_BOOL_LITERAL:
            if (typed) snprintf(buf, bufsz, "<%s> : %s", node->data.bool_literal.value ? "true" : "false", datatype_to_string(node->data_type));
            else snprintf(buf, bufsz, "<%s>", node->data.bool_literal.value ? "true" : "false");
            break;
        case NODE_STRING_LITERAL:
            if (typed) snprintf(buf, bufsz, "<\"%s\"> : %s", node->data.string_literal.value, datatype_to_string(node->data_type));
            else snprintf(buf, bufsz, "<\"%s\">", node->data.string_literal.value);
            break;
    ...
    switch (node->type) {
        case NODE_DECL:
            if (node->data.decl.init) {
                char idbuf[128];
                snprintf(idbuf, sizeof idbuf, "<%s>", node->data.decl.name);
                print_branch_line(prefix, 0, idbuf);
        ...
        case NODE_ASSIGN: {
            char idbuf[128];
            snprintf(idbuf, sizeof idbuf, "<%s>", node->data.assign.name);
            print_branch_line(prefix, 0, idbuf);
