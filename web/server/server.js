...
function binaryPath(langKey) {
  ...
  return path.join(ROOT, l.dir, name);
}
...
app.post("/api/compile", (req, res) => {
  ...
  if (!fs.existsSync(bin)) {
    return res.status(500).json({
      error: `Compiler binary not built: ${bin}. Run "make" inside ${lang.dir}/ first.`,
    });
  }
  ...
  const child = spawn(bin, [tmpFile], { cwd: path.join(ROOT, lang.dir) });
  let stdout = "";
  let stderr = "";
  let timedOut = false;
